...
//            }
        ...
        getJSF: function( text ){
            ...
            return ( new LogicTplUnit( new Function('vars', jsCode) ) );
        },
        tplList: {},
        renderers: {},
        loadTpl: function( dirName, name ){
            this.tplList[name] = this.getJSF( fs.readFileSync( dirName +'/'+ name +'.html', 'utf-8' ) );
            this.renderers[name] = this.render.bind(this, name);
        },
        render: function( name, obj ){
            var tpl = this.tplList[ name ];
            return !tpl ? null : tpl.f(obj || {});
        },
        loadAll: function( dirName ){
            dirName = dirName || 'tpl';
            fs.readdirSync(dirName).forEach(function( fullName ){
                var name = fullName.replace('.html','');
                this.loadTpl(dirName, name);
            }.bind(this));
        }
    };
